...
\title{Spadina Paper\thanks{Code and data are available at:
https://github.com/Sluuu/Politics.git}}
\usepackage{etoolbox}
\makeatletter
...
we are interested in the US political support. In particular we are
interested in whether we can forecast who a respondent is likely to vote
for, based on knowing their employment status, highest level of
education, and race. That means we are interested in a dataset with
variables for who an individual voted for, and some of their
characteristics, such as employment status, education level, and their
race. We use R Core Team (2023) and Wickham et al. (2019).
...
Some of our data is of penguins (\textbf{?@fig-bills}), from
...
\mbox{logit}(\pi_i) &= \alpha + \beta_1 \times \mbox{employment_status}_i + \beta_2 \times \mbox{education}_i + \beta_3 \times \mbox{race}_i + \beta_4 \times \mbox{gender}_i\\
\alpha &\sim \mbox{Normal}(0, 2.5) \\
\beta_1 &\sim \mbox{Normal}(0, 5.02) \\
\beta_2 &\sim \mbox{Normal}(0, 6.34) \\
\beta_3 &\sim \mbox{Normal}(0, 7.4) \\
\beta_4 &\sim \mbox{Normal}(0, 8.6)
\end{aligned}
\]
...
\includegraphics{paper_files/figure-pdf/fig-modelresultsfig-1.pdf}

}

\caption{\label{fig-modelresultsfig}Explanatory models of flight time
based on wing width and wing length}
